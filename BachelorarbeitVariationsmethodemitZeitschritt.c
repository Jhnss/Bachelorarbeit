...
int main(void)
{
    ...
    for(int i=0;i<n;i++)
	{
		vektorLoesung[i] = sin(M_PI/(2*R)*(-R+(i+1)*h))- (-R+(i+1)*h)/R;
	}
    ...
    for(int k=0;k<zeitschritte;k++)
    {
        ...
        for(int j=0;j<wiederholungen;j++)
        {
            ...
            //-H wird erstellt
            vektorNegH[0] = -(vektorLoesung[0]*matrixA_diagnonaleintrag+matrixA_nichtDiagonaleintrag*vektorLoesung[1]
                        -(1/(tau*2*h*(vektorUk[0]/h+1/h))*(h*pot(vektorUk[0], 2)
                        -1/(3*(vektorUk[0]/h+1/h))*(pot(vektorUk[0], 3)-pot(-1, 3)))
                        -1/(2*h*4*(vektorLoesung[0]/h+1/R))*(h*pot(vektorLoesung[0]+(-R+h)/R, 4)
                        -1/(5*(vektorLoesung[0]/h+1/R))*(pot(vektorLoesung[0]+(-R+h)/R, 5)-pot(-R/R, 5)))
                        -1/h*(1/tau-0.5)*(pot(-R+h, 3)/(3*R)-((-R)*pot(-R+h, 2))/(2*R)+pot(-R, 3)/(6*R)))
                        -(-1*(1/(tau*2*h*(vektorUk[1]/h-vektorUk[0]/h))*(h*pot(vektorUk[1], 2)
                        -1/(3*(vektorUk[1]/h-vektorUk[0]/h))*(pot(vektorUk[1], 3)-pot(vektorUk[0], 3)))
                        -1/(2*h*4*(vektorLoesung[1]/h-vektorLoesung[0]/h+1/R))*(h*pot(vektorLoesung[1]+(-R+2*h)/R, 4)
                        -1/(5*(vektorLoesung[1]/h-vektorLoesung[0]/h+1/R))*(pot(vektorLoesung[1]+(-R+2*h)/R, 5)
                        -pot(vektorLoesung[0]+(-R+h)/R, 5)))-1/h*(1/tau-0.5)*(pot(-R+2*h, 3)/(3*R)
                        -((-R+h)*pot(-R+2*h, 2))/(2*R)+pot(-R+h, 3)/(6*R)))+1/(2*tau*(vektorUk[1]/h-vektorUk[0]/h))*(pot(vektorUk[1], 2)-pot(vektorUk[0], 2))
                        -1/(8*(vektorLoesung[1]/h-vektorLoesung[0]/h+1/R))*(pot(vektorLoesung[1]+(-R+2*h)/R, 4)-pot(vektorLoesung[0]+(-R+h)/R, 4))
                        -(1/tau-0.5)*(pot(-R+2*h, 2)/(2*R)-pot(-R+h, 2)/(2*R))));
            for(int i=1;i<n-1;i++)
            {
                vektorNegH[i] = -(vektorLoesung[i]*matrixA_diagnonaleintrag+matrixA_nichtDiagonaleintrag*(vektorLoesung[i-1]+vektorLoesung[i+1])
                            -(1/(tau*2*h*(vektorUk[i]/h-vektorUk[i-1]/h))*(h*pot(vektorUk[i], 2)
                            -1/(3*(vektorUk[i]/h-vektorUk[i-1]/h))*(pot(vektorUk[i], 3)-pot(vektorUk[i-1], 3)))
                            -1/(2*h*4*(vektorLoesung[i]/h-vektorLoesung[i-1]/h+1/R))*(h*pot(vektorLoesung[i]+(-R+(i+1)*h)/R, 4)
                            -1/(5*(vektorLoesung[i]/h-vektorLoesung[i-1]/h+1/R))*(pot(vektorLoesung[i]+(-R+(i+1)*h)/R, 5)
                            -pot(vektorLoesung[i-1]+(-R+i*h)/R, 5)))-1/h*(1/tau-0.5)*(pot(-R+(i+1)*h, 3)/(3*R)
                            -((-R+i*h)*pot(-R+(i+1)*h, 2))/(2*R)+pot(-R+i*h, 3)/(6*R)))
                            -(-1*(1/(tau*2*h*(vektorUk[i+1]/h-vektorUk[i]/h))*(h*pot(vektorUk[i+1], 2)
                            -1/(3*(vektorUk[i+1]/h-vektorUk[i]/h))*(pot(vektorUk[i+1], 3)-pot(vektorUk[i], 3)))
                            -1/(2*h*4*(vektorLoesung[i+1]/h-vektorLoesung[i]/h+1/R))*(h*pot(vektorLoesung[i+1]+(-R+(i+2)*h)/R, 4)
                            -1/(5*(vektorLoesung[i+1]/h-vektorLoesung[i]/h+1/R))*(pot(vektorLoesung[i+1]+(-R+(i+2)*h)/R, 5)
                            -pot(vektorLoesung[i]+(-R+(i+1)*h)/R, 5)))-1/h*(1/tau-0.5)*(pot(-R+(i+2)*h, 3)/(3*R)
                            -((-R+(i+1)*h)*pot(-R+(i+2)*h, 2))/(2*R)+pot(-R+(i+1)*h, 3)/(6*R)))
                            +1/(2*tau*(vektorUk[i+1]/h-vektorUk[i]/h))*(pot(vektorUk[i+1], 2)
                            -pot(vektorUk[i], 2))-1/(8*(vektorLoesung[i+1]/h-vektorLoesung[i]/h+1/R))*(pot(vektorLoesung[i+1]
                            +(-R+(i+2)*h)/R, 4)-pot(vektorLoesung[i]+(-R+(i+1)*h)/R, 4))
                            -(1/tau-0.5)*(pot(-R+(i+2)*h, 2)/(2*R)-pot(-R+(i+1)*h, 2)/(2*R))));
            }
            vektorNegH[n-1] = -(vektorLoesung[n-1]*matrixA_diagnonaleintrag+matrixA_nichtDiagonaleintrag*vektorLoesung[n-2]
                          -(1/(tau*2*h*(vektorUk[n-1]/h-vektorUk[n-2]/h))*(h*pot(vektorUk[n-1], 2)
                        -1/(3*(vektorUk[n-1]/h-vektorUk[n-2]/h))*(pot(vektorUk[n-1], 3)-pot(vektorUk[n-2], 3)))
                        -1/(2*h*4*(vektorLoesung[n-1]/h-vektorLoesung[n-2]/h+1/R))*(h*pot(vektorLoesung[n-1]
                        +(-R+n*h)/R, 4)-1/(5*(vektorLoesung[n-1]/h-vektorLoesung[n-2]/h+1/R))*(pot(vektorLoesung[n-1]
                        +(-R+n*h)/R, 5)-pot(vektorLoesung[n-2]+(-R+(n-1)*h)/R, 5)))-1/h*(1/tau-0.5)*(pot(-R+n*h, 3)/(3*R)
                        -((-R+(n-1)*h)*pot(-R+n*h, 2))/(2*R)+pot(-R+(n-1)*h, 3)/(6*R)))
                        -(-1*(1/(tau*2*h*(1/h-vektorUk[n-1]/h))*(h*pot(1, 2)
                        -1/(3*(1/h-vektorUk[n-1]/h))*(pot(1, 3)-pot(vektorUk[n-1], 3)))-1/(2*h*4*(-vektorLoesung[n-1]/h+1/R))*(h*pot((-R+(n+1)*h)/R, 4)
                        -1/(5*(-vektorLoesung[n-1]/h+1/R))*(pot((-R+(n+1)*h)/R, 5)-pot(vektorLoesung[n-1]+(-R+n*h)/R, 5)))
                        -1/h*(1/tau-0.5)*(pot(-R+(n+1)*h, 3)/(3*R)-((-R+n*h)*pot(-R+(n+1)*h, 2))/(2*R)+pot(-R+n*h, 3)/(6*R)))
                        +1/(2*tau*(1/h-vektorUk[n-1]/h))*(pot(1, 2)-pot(vektorUk[n-1], 2))
                        -1/(8*(-vektorLoesung[n-1]/h+1/R))*(pot((-R+(n+1)*h)/R, 4)-pot(vektorLoesung[n-1]+(-R+n*h)/R, 4))
                        -(1/tau-0.5)*(pot(-R+(n+1)*h, 2)/(2*R)-pot(-R+n*h, 2)/(2*R))));
